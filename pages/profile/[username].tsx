...
export default function UserProfile() {
  ...
  if (!username) return null; // enquanto o parâmetro não está disponível

  const decodedUsername = decodeURIComponent(username as string);
  const user = usersData.users.find((u: any) => u.name === decodedUsername);

  if (!user) {
    return (
      <Box p="40px">
        <Text fontSize="24px">Usuário não encontrado</Text>
      </Box>
    );
  }
  ...
  return (
    <Box px="180px" py="40px" mx="auto">
      ...
      {/* Exibe o ProfileHeading passando os dados do usuário encontrado */}
      <ProfileHeading isMyProfile={false} userData={user} />
